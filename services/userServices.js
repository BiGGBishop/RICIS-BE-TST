...
exports.signUpUsers = async (req) => {
  ...
  // Create the new user
  const newUser = await UserRepo.createUser(userObject);
...
exports.recoveryOtp = async (req) => {
  ...
  if (user_type === 'user') {
    userExist = await UserRepo.findUser({ email });
  } else if (user_type === 'staff') {
    userExist = await AdminRepo.findAdminStaff({ email });
  } else {
    return {
      STATUS_CODE: StatusCodes.BAD_REQUEST,
      STATUS: false,
      MESSAGE: "Invalid user_type. Must be 'user' or 'staff'.",
    };
  }
...
//     userExist = await UserRepo.findUser({ email });
//   } else if (user_type === 'Staff') {
//     userExist = await AdminRepo.findUser({ email });
//   } else {
//     return {
//       STATUS_CODE: StatusCodes.BAD_REQUEST,
//       STATUS: false,
