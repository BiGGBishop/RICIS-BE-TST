...
exports.createAuthorizationApproved = async (req) => {
  ...
  const data = {
    ...
    companyQualityManual: await  uploadSingleFile(req.file.companyQualityManual[0]) || null,
    operationalProcedures: await uploadSingleFile(req.file.operationalProcedures[0]) || null,
    companyDocumentation: await uploadSingleFile(req.file.companyDocumentation[0]) || null,
    documentationSupervisor: await uploadSingleFile(req.file.documentationSupervisor[0]) || null,
    isoCertification: await uploadSingleFile(req.file.isoCertification[0]) || null,
